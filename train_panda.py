...
def get_args():
    ...
    args.data_folder = 'DREAM_ds/panda-orb'
    args.base_dir = "/CtRNet-robot-pose-estimation"
    args.use_gpu = True
    args.trained_on_multi_gpus = True
    args.keypoint_seg_model_path = os.path.join(args.base_dir,"pretrain/net_best.pth")
...
for phase in ['train','valid']:
    if phase == 'train':
        datasets[phase] = ImageDataLoaderReal(data_folder = ["DREAM_ds/panda-3cam_kinect360", 
                                                            "DREAM_ds/panda-3cam_azure", 
                                                            "DREAM_ds/panda-3cam_realsense", 
                                                            "DREAM_ds/panda-orb"], 
...
#load the best weight from first 100 epochs training
best_weight_path = "outputs_Aug_10_self_training/panda-orb/net_best.pth"
checkpoint = torch.load(best_weight_path)
CtRNet.keypoint_seg_predictor.load_state_dict(checkpoint)
print(f"loaded self training weight from {best_weight_path}")

# make the training resumable
start_epoch = 0
checkpoint_path = os.path.join(args.out_dir, 'net_20.pth')
if os.path.exists(checkpoint_path):
    print("Loading checkpoint from '{}'...".format(checkpoint_path))
    checkpoint = torch.load(checkpoint_path)
    CtRNet.keypoint_seg_predictor.load_state_dict(checkpoint['model_state_dict'])
    optimizer.load_state_dict(checkpoint['optimizer_state_dict'])
    scheduler.load_state_dict(checkpoint['scheduler_state_dict'])
    start_epoch = checkpoint['epoch']
    best_AUC_ADD = checkpoint['best_AUC_ADD']
    print("Checkpoint loaded: starting from epoch {}, best_AUC_ADD: {}".format(start_epoch, best_AUC_ADD))
        ...
        if phase == 'train':
            ...
            for i, data in tqdm(enumerate(loader), total=data_n_batches[phase]):
                ...
                with torch.set_grad_enabled(phase == 'train'):
                    ...
                    points_2d = points_2d[:, 6:, :] if epoch % 2 ==0 else points_2d[:, :6, :]  #train base & ee separately
                    ...
                    for b in range(img.shape[0]):
                        ...
                        points_3d = points_3d[6:] if epoch % 2 == 0 else points_3d[:6] #train base & ee separately
                        # print(f"check 3d and 2d shape: {points_3d.shape} and {points_2d.shape}")
                        # solve for pose
        ...
        if phase == 'valid':
            ...
            if AUC_ADD > best_AUC_ADD:
                ...
                torch.save({'epoch': epoch,
                    'model_state_dict': CtRNet.keypoint_seg_predictor.state_dict(),
                    'optimizer_state_dict': optimizer.state_dict(),
                    'scheduler_state_dict': scheduler.state_dict(),
                    'best_AUC_ADD': best_AUC_ADD}, 
